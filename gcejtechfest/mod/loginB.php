<?php 
    ...
    if(isset($_POST['submit']))
    {
            ...
            $pass=$_POST['password'];
            $sel="SELECT * FROM loginb WHERE user='$username' AND pass='$pass';";
            $e_sel=mysqli_query($sql,$sel);
            $row=mysqli_fetch_array($e_sel);
            if($row['user']==$username && $row['pass']==$pass)
            {
                    $_SESSION['loginb']=$username;
                    header('Location:Group-B.php');
            }
            else
            {
                $_SESSION['loginb']="fail";
            }
    }
?>
<!DOCTYPE html>
              ...
              <div class="col-lg-12">
                <div class="p-5">
                  <div class="text-center">
                    <h1 class="h4 text-gray-900 mb-4">Login Moderator - Group B</h1>
                  </div>
                  <form class="user" action="loginB.php" method="POST">
