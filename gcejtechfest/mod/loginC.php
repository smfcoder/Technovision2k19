<?php 
    ...
    if(isset($_POST['submit']))
    {
            ...
            $pass=$_POST['password'];
            $sel="SELECT * FROM loginc WHERE user='$username' AND pass='$pass';";
            $e_sel=mysqli_query($sql,$sel);
            $row=mysqli_fetch_array($e_sel);
            if($row['user']==$username && $row['pass']==$pass)
            {
                    $_SESSION['loginc']=$username;
                    header('Location:Group-C.php');
            }
            else
            {
                $_SESSION['loginc']="fail";
            }
    }
?>
<!DOCTYPE html>
